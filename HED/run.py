...
arguments_strIn = './1500'
arguments_strOut = './results'
...
if __name__ == '__main__':
	for curDir, dirs, files in os.walk(arguments_strIn):
    		for file in files:
                        tensorInput = torch.FloatTensor(numpy.array(PIL.Image.open(os.path.join(curDir, file)))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
                        print(file)
                        tensorOutput = estimate(tensorInput)
                        PIL.Image.fromarray((tensorOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8)).save(os.path.join(arguments_strOut, file))
# end
